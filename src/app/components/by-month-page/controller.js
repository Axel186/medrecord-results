...
module.exports = ["BloodProcedure", "$timeout", function(BloodProcedure, $timeout) {
  ...
  BloodProcedure.loadAllData().then(function(data) {
    ...
    self.data = [{
      key: "Diastolic",
      bar: true,
      values: BloodProcedure.collection.diastolic.avgByMonth.map(function(item) {
        var timestamp = moment(item.label, "MM-YYYY")._d;
        return [
          timestamp,
          item.value
        ]
      ...
      key: "Systolic",
      values: BloodProcedure.collection.systolic.avgByMonth.map(function(item) {
        var timestamp = moment(item.label, "MM-YYYY")._d;
